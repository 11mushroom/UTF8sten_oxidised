...
use utf8sten::{char_slice_to_utf8_unchecked, enSten2, enSten2_to, supports_v2_encode};

// buffer size must be 2*x for block encoding with v2
...
fn main() {
    ...
    if args.len()<2 {
      ...
      'enc_l:loop{
        ...
        if !supports_v2_encode(&buff[..read_len]) {
      ...
      //checks if data can be encoded in a valid output
      if !supports_v2_encode(args[1].as_bytes()) {
        eprintln!("cannot be encoded with second version");
